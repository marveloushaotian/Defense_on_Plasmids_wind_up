...
# 读取新文件
data <- read.csv("Collect/all_intermediate_files_by_defense_type.csv")

# 重新获取唯一的 Type_Group
complete_y <- unique(data$Type_Group)
...
generate_plot <- function(value_group, x_label, x_limits, title) {
  ...
  plot_data <- plot_data %>%
    complete(Type_Group = complete_y, fill = list(Value = NA))
  
  p <- ggplot(plot_data, aes(x = Value, y = Type_Group)) +
...
# 保存为 PDF
output_pdf <- "updated_combined_plots_by_defense_type.pdf"

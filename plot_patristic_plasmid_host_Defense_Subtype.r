...
if (nrow(all_data) > 0) {
  write.csv(all_data, "Collect/Defense_Subtype_Group/intermediate_data_for_pairwise_patristic_distance_of_plasmid_host_plot.csv", row.names = FALSE)
}
...
if (nrow(all_data) > 0) {
  ...
  ggsave(filename = "Results/Defense_Subtype_Group/pairwise_patristic_distance_of_plasmid_host.pdf", 
       plot = p, 
       width = 8,
       height = 100,
       dpi = 600,
